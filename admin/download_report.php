...
if ($type == 'All') {
    // Show all House and PG/Room bookings
    $query = "
 ...
 elseif ($type == 'House') {
    // Show only House bookings
    $query = "
        SELECT pg.pg_id as house_id, pg.pg_name as house_name, pg.pg_address as room_address, customer.*, booking.*, 
        (25 / 100) * (pg.pg_price * booking.stay) as advance_price, booking.book_status
        FROM booking
        JOIN customer ON booking.cus_id = customer.login_id
        JOIN add_pg as pg ON booking.room_id = pg.pg_id";
} else {
    // Show only PG/Room bookings
    $query = "
